...
int SportsLayout::find_contribution(int *mp, int idx)
{
  ...
  void SportsLayout::simulated_annealing(int* best_mp,int &best_cost){
        ...
        auto next_state_random = [&](vector<int> &unused_locations,int &iterations){
            int idx = rnd2(gen);
            int loc = rnd3(gen);
            ...
            if(final_contribution<initial_contribution || exp((initial_contribution-final_contribution)/iterations*1.0)>=threshold){
        ...
        while(restarts--){
            ...
            int it=iterations;
            while(iterations--){
                next_state_random(unused_locations,iterations);
                ...
                //     // else threshold = (double)(it-iterations)/(double)(it*1.0);
                // }
            }
